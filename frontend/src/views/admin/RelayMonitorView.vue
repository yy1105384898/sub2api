  ...
  <AppLayout>
    <div class="space-y-5">
      ...
      <div v-show="activeTab === 'overview'" class="space-y-4">
        ...
        <div v-if="overviewSites.length" class="grid grid-cols-1 gap-4 xl:grid-cols-2">
          <section
            v-for="(site, siteIndex) in overviewSites"
            :key="site.key"
            class="rounded-lg border p-4"
            :class="[siteTone(siteIndex).site, siteTone(siteIndex).border]"
          >
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                  <h3 class="truncate text-lg font-semibold text-gray-950 dark:text-white">{{ site.site }}</h3>
                  <span class="rounded bg-white/70 px-2 py-0.5 text-xs text-gray-600 dark:bg-dark-950/40 dark:text-gray-300">{{ site.system }}</span>
                  <span class="rounded bg-white/70 px-2 py-0.5 text-xs text-gray-600 dark:bg-dark-950/40 dark:text-gray-300">{{ site.vendor || t('admin.relayMonitor.noVendor') }}</span>
                </div>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.relayMonitor.latestChange') }}:
                  {{ site.lastChangedAt ? formatTime(site.lastChangedAt) : t('admin.relayMonitor.noChange') }}
                </p>
              </div>
              <div class="grid grid-cols-3 gap-2 text-center text-xs">
                <div class="rounded bg-white/70 px-3 py-2 dark:bg-dark-950/40">
                  <p class="text-gray-500 dark:text-gray-400">{{ t('admin.relayMonitor.totalGroups') }}</p>
                  <p class="text-base font-semibold text-gray-900 dark:text-white">{{ site.rows.length }}</p>
                </div>
                <div class="rounded bg-red-50 px-3 py-2 text-red-700 dark:bg-red-950/30 dark:text-red-300">
                  <p>{{ t('admin.relayMonitor.up') }}</p>
                  <p class="text-base font-semibold">{{ site.upCount }}</p>
                </div>
                <div class="rounded bg-green-50 px-3 py-2 text-green-700 dark:bg-green-950/30 dark:text-green-300">
                  <p>{{ t('admin.relayMonitor.down') }}</p>
                  <p class="text-base font-semibold">{{ site.downCount }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="mt-4 space-y-3">
              <div
                v-for="section in site.planSections"
                :key="section.key"
                class="rounded-lg border p-3"
                :class="[section.tier.panelCls, section.tier.borderCls]"
              >
                <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
                  <div class="flex flex-wrap items-center gap-2">
                    <span class="rounded bg-white/70 px-2 py-0.5 text-xs font-medium text-gray-700 dark:bg-dark-950/40 dark:text-gray-200">{{ section.vendor }}</span>
                    <span class="rounded px-2 py-0.5 text-xs font-medium" :class="section.tier.cls">{{ section.tier.label }}</span>
                  </div>
                  <span class="text-xs text-gray-500 dark:text-gray-400">{{ section.rows.length }} {{ t('admin.relayMonitor.groupsUnit') }}</span>
                </div>
                ...
                <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
                  <article
                    ...
                    class="rounded border border-white/60 bg-white/75 p-3 dark:border-dark-600/70 dark:bg-dark-900/65"
                  >
                    <div class="flex items-start justify-between gap-3">
                      <div class="min-w-0">
                        <p class="truncate text-sm font-semibold text-gray-900 dark:text-white">{{ row.group_name }}</p>
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ displayPlan(row) }}</p>
                      </div>
                      <span
                      ...
                      >
                        {{ row.has_change ? (row.direction === 'up' ? t('admin.relayMonitor.up') : t('admin.relayMonitor.down')) : t('admin.relayMonitor.stable') }}
                      </span>
                    </div>
                    <div class="mt-3 flex items-end justify-between gap-3">
                      <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.relayMonitor.colCurrentRate') }}</p>
                        <p class="text-xl font-bold text-gray-950 dark:text-white">{{ formatRate(row.current_rate) }}</p>
                      </div>
                      <div class="text-right text-xs">
                        <p v-if="row.has_change" :class="row.direction === 'up' ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'">
                          {{ formatRate(row.old_rate) }} → {{ formatRate(row.new_rate) }}
                        </p>
                        <p class="mt-1 text-gray-400">{{ row.changed_at ? formatTime(row.changed_at) : '-' }}</p>
                      </div>
                    </div>
                  </article>
                </div>
...
interface OverviewSite {
  ...
  planSections: OverviewPlanSection[]
}
...
const SITE_TONES: SiteTone[] = [
  { site: 'bg-sky-50/80 dark:bg-sky-950/20', border: 'border-sky-100 dark:border-sky-900/40' },
  { site: 'bg-violet-50/80 dark:bg-violet-950/20', border: 'border-violet-100 dark:border-violet-900/40' },
  { site: 'bg-emerald-50/80 dark:bg-emerald-950/20', border: 'border-emerald-100 dark:border-emerald-900/40' },
  { site: 'bg-rose-50/80 dark:bg-rose-950/20', border: 'border-rose-100 dark:border-rose-900/40' },
  { site: 'bg-cyan-50/80 dark:bg-cyan-950/20', border: 'border-cyan-100 dark:border-cyan-900/40' },
  { site: 'bg-lime-50/80 dark:bg-lime-950/20', border: 'border-lime-100 dark:border-lime-900/40' },
]
...
const overviewSites = computed<OverviewSite[]>(() => {
  ...
  return Array.from(siteMap.entries()).map(([key, rows]) => {
    ...
    const sectionMap = new Map<string, RelayOverviewRow[]>()
    for (const row of sortedRows) {
      const tier = planTier(row.group_name)
      const vendor = normalizeVendor(row.vendor)
      const sectionKey = `${vendor}:${tier.key}`
      const sectionRows = sectionMap.get(sectionKey) ?? []
      sectionRows.push(row)
      sectionMap.set(sectionKey, sectionRows)
    }
    const planSections = Array.from(sectionMap.entries()).map(([sectionKey, sectionRows]) => {
      const row = sectionRows[0]
      return {
        key: sectionKey,
        vendor: normalizeVendor(row.vendor),
        tier: planTier(row.group_name),
        rows: sectionRows,
      }
    })
    ...
    return {
      ...
      vendor: first.vendor,
      rows: sortedRows,
      upCount: rows.filter((r) => r.direction === 'up').length,
      downCount: rows.filter((r) => r.direction === 'down').length,
...
function displayPlan(row: RelayOverviewRow | RelayRateChange): string {
  return `${normalizeVendor(row.vendor)} / ${planTier(row.group_name).label}`
}
